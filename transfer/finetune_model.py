...
checkpoint_callback = CheckpointCallback(
  save_freq=2500,
  save_path="./logs/",
  name_prefix="chaser_arpod",
  save_replay_buffer=True,
  save_vecnormalize=True,
)

callback = CallbackList([CheckpointCallback(save_freq=2500, save_path="./logs/", name_prefix="chaser_arpod", save_replay_buffer=True, save_vecnormalize=True)])



env = make_vec_env(ARPOD_GYM, wrapper_class=TimeLimitWrapper, n_envs=100)
stats_path = os.path.join('envstats', 'chaser1.pkl')
...
model_dir = f'models/chaser1.zip'

policy_dict = {'activation_fn': GELU, 'net_arch': {'pi': [130, 44, 30], 'vf': [145, 25, 5]}, 'full_std': False, 'squash_output': True, 'log_std_init': -2.3, 'use_expln' : True}
...
"""
loaded_model = PPO.load(model_dir, env=normalized_vec_env, print_system_info=True, force_reset=False)


new_model = PPO(MlpPolicy, env=normalized_vec_env, 
                                   gamma=0.998,
                                   gae_lambda=0.95,
                                   learning_rate=0.03,
                                   n_steps=500,
